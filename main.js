function doGet(e){
  ...
  if(page == "index" || page==null){
    var ssh = SpreadsheetApp.openById('1_q2Yua0uX3ag5W4uDyzp-FH-WL2yNQtL5TpA0K9mGAc'); //スタンドアローン化したSpreadsheetに接続
    ...
    const sheet = SpreadsheetApp.openById('1_q2Yua0uX3ag5W4uDyzp-FH-WL2yNQtL5TpA0K9mGAc');//スタンドアローン化したSpreadsheetに接続
    value = sheet.getRange('A2').getValue();//現在の入室人数を保存しているセル'A2'の値を取得し，value変数に代入
    value = value + 1;//value変数に1を加算して，value変数を更新
    value = sheet.getRange('A2').setValue(value);//更新されたvalue変数をセル'B2'に代入し条件分岐で0以下にならないようにする（スプレッドシートで計算）
